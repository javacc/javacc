...
```
$ unzip javacc-7.0.7.zip
or
$ tar xvf javacc-7.0.7.tar.gz
```

Then place the binary `javacc-7.0.7.jar` in a new `target/` folder, and rename to `javacc.jar`.
...
On UNIX based systems, the scripts may not be executable immediately. This can be solved by using the command from the `javacc-7.0.7/` directory:
