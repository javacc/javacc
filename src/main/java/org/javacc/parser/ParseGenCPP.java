...
//          genCodeLine("      cerr << "\"Parse error at:  << std::to_string(token->beginLine()) << ':' << std_to_string(token->beginColumn()) << \", after token: \" << addUnicodeEscapes(token->image()) << \" encountered: << addUnicodeEscapes(getToken(1)->image());");"
      }
      genCodeLine("   }");
