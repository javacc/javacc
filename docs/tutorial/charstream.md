...
| <p><code>ASCII_UCodeESC_CharStream</p></code> | Generated when the option `JAVA_UNICODE_ESCAPE` is set and the `UNICODE_INPUT` option is not set. | This class treats the input as a stream of 1-byte characters. However, the special escape sequence:<br><p><code>("\\\\")* "\\" ("u")+ // odd number of backslashes followed by one or more u's</p></code><br>
